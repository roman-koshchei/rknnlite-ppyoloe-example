...
import argparse
import numpy as np
import time  # for timing post_process
...
def dfl(position):
    """Distribution Focal Loss (DFL) implementation without PyTorch.

    Args:
        position (np.ndarray): shape (N, C, H, W), where C = p_num * mc.

    Returns:
        np.ndarray: aggregated bbox distance predictions with shape (N, p_num, H, W).
    """
    # We implement the same logic as the original PyTorch version using NumPy to
    # avoid importing the heavy `torch` package at runtime.
    x = position  # already NumPy array coming from RKNN
    ...
    softmax_y = exp_y / exp_y.sum(axis=2, keepdims=True)

    # Create accuracy matrix [0, 1, 2, ..., mc-1]
    acc_matrix = np.arange(mc, dtype=np.float32).reshape(1, 1, mc, 1, 1)

    # Weighted sum across mc dimension
    y_out = (softmax_y * acc_matrix).sum(axis=2)
...
if __name__ == "__main__":
    ...
    for i in range(len(img_list)):
        ...
        if platform in ["pytorch", "onnx"]:
            ...
            # RKNN requires a 4-D NHWC input (batch, height, width, channels).
            # Add a batch dimension with numpy for clarity.
            input_data = np.expand_dims(img, axis=0)

        outputs = model.run([input_data])

        # measure post_process execution time
        _t0 = time.perf_counter()
        boxes, classes, scores = post_process(outputs)
        _elapsed = (time.perf_counter() - _t0) * 1000.0  # ms
        print(f"post_process time: {_elapsed:.2f} ms", end="\r")
